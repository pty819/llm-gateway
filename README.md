...
- Admin-only PostgreSQL analytics endpoints for manual heavy usage review.
...
Admin heavy analytics run directly against PostgreSQL (optionally a read-only replica via `LLM_GATEWAY_ANALYTICS_DATABASE_URL`) for longer time windows, and are guarded by a per-transaction `statement_timeout`. Normal user self-usage stays PostgreSQL-backed so it remains fresh and subject-scoped.
