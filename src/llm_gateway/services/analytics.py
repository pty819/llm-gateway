...
def _round2_avg(column):
    ...
    PG 不接受 ROUND(double, int)，必须先 cast 成 numeric。
    结果保留 2 位小数的数值。
    """
...
def _full_metric_columns() -> list:
    ...
    cached_tokens 在 success_count 之前，所以 full 不能写成
    core + 扩展——字段顺序不同。独立定义。
    """
    total_tokens_expr = func.coalesce(
        RequestFact.total_tokens,
...
async def _apply_statement_timeout(session) -> None:
    ...
    monopolize the shared primary connection. 大窗口聚合若无此守卫会压垮数据面。
...
async def usage_totals(
    ...
    """全量 18-metric 单行聚合。无数据返回 None（request_count==0→None）。"""
...
async def drilldown(
    ...
    *,
    ...
    """按 dimension 分组，full 18-metric。dimension_id 转 str。
