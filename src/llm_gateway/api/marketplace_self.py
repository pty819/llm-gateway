"""Self-service marketplace registry routes: skills + mcps (publish, list,
browse, download, like, grants).

Sub-router has NO prefix — it is included by ``api/auth.py`` under the shared
``/auth`` prefix, so paths compose to ``/auth/registry/...`` unchanged.
"""
...
async def require_owned_skill(session, ctx, slug, include_disabled=False):
...
async def require_owned_mcp(session, ctx, slug, include_disabled=False):
...
async def list_my_skill_grants(
    ...
    skill = await require_owned_skill(session, ctx, slug)
...
async def list_my_mcp_grants(
    ...
    mcp = await require_owned_mcp(session, ctx, slug)
